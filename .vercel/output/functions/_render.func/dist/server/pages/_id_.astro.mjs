import { e as createComponent, f as createAstro, k as renderComponent, r as renderTemplate, m as maybeRenderHead, l as Fragment, h as addAttribute } from '../chunks/astro/server_CEVrwVh7.mjs';
import { g as getLinkById, a as getLevelOneItemsByLink, b as getAllLinks, c as getCategories, d as getLinkColorIndex, e as candyColors, $ as $$Layout, f as $$MeshGradient, h as $$Icon, i as cn, j as getColorByIndex } from '../chunks/utils_Dqgdi9QZ.mjs';
...
const $$id = createComponent(async ($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${renderComponent($$result, "Layout", $$Layout, { "title": linkData?.name ? `${linkData.name} - alditpra` : "alditpra" }, { "default": async ($$result2) => renderTemplate` ${maybeRenderHead()}<main class="min-h-screen relative overflow-hidden bg-[#fafafa]"> ${renderComponent($$result2, "MeshGradient", $$MeshGradient, {})} <div class="relative z-10 container mx-auto px-4 py-8 sm:py-12 max-w-4xl">  ${hasError && renderTemplate`<div class="max-w-2xl mx-auto text-center py-20"> <div class="bg-white/60 backdrop-blur-xl rounded-3xl p-8 sm:p-12 border border-white/60 shadow-xl"> <div class="w-20 h-20 mx-auto mb-6 rounded-2xl bg-red-100 flex items-center justify-center"> ${renderComponent($$result2, "Icon", $$Icon, { "name": "lucide:alert-triangle", "class": "w-10 h-10 text-red-600" })} </div> <h2 class="text-2xl sm:text-3xl font-bold text-zinc-800 mb-3"> ${errorMessage} </h2> <p class="text-zinc-600 mb-6 leading-relaxed">
Terjadi kesalahan saat mengambil data dari Google Sheets.
</p> <div class="flex gap-3 justify-center"> <a href="/" class="inline-flex items-center gap-2 px-6 py-3 bg-white border border-zinc-300 text-zinc-700 font-semibold rounded-2xl hover:bg-zinc-50 transition-colors duration-300"> ${renderComponent($$result2, "Icon", $$Icon, { "name": "lucide:home", "class": "w-5 h-5" })}
Kembali ke Home
</a> <button onclick="window.location.reload()" class="inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-candy-purple to-candy-pink text-white font-semibold rounded-2xl hover:scale-105 transition-transform duration-300 shadow-lg"> ${renderComponent($$result2, "Icon", $$Icon, { "name": "lucide:refresh-cw", "class": "w-5 h-5" })}
Coba Lagi
</button> </div> </div> </div>`}  ${!hasError && linkData && renderTemplate`${renderComponent($$result2, "Fragment", Fragment, {}, { "default": async ($$result3) => renderTemplate`<div class="mb-8">  <a href="/" class="inline-flex items-center gap-3 mb-6 group"> <div${addAttribute(cn(
...
Isi Folder
      ...
      ), "class")}${addAttribute(`--hover-bg: ${pageColor.glow}15;`, "style")}> ${renderComponent($$result3, "Icon", $$Icon, { "name": "lucide:external-link", "class": cn("w-4 h-4 transition-colors", pageColor.text) })} <span${addAttribute(cn("font-semibold text-sm transition-colors hidden sm:inline", pageColor.text), "class")}>
Buka di Tab Baru
...
Buka
    ...
    return renderTemplate`<div class="grid gap-4"> ${items.map((item, index) => {
      ...
      return renderTemplate`<a${addAttribute(item.link, "href")} target="_blank" rel="noopener noreferrer" class="group block bg-white/60 hover:bg-white backdrop-blur-sm border border-white/80 hover:border-transparent rounded-2xl p-3 sm:p-4 transition-all duration-300 shadow-sm hover:shadow-xl hover:-translate-y-1"> <div class="flex items-center gap-3">  <div${addAttribute(cn(
        ...
        color.bg
      ), "class")}${addAttribute(`box-shadow: 0 4px 15px -3px ${color.glow};`, "style")}> ${item.icon ? renderTemplate`${renderComponent($$result3, "Icon", $$Icon, { "name": `lucide:${item.icon}`, "class": cn("w-5 h-5", color.text) })}` : renderTemplate`<span${addAttribute(cn("text-lg font-bold", color.text), "class")}> ${item.title.charAt(0).toUpperCase()} </span>`} </div> <div class="flex-1 min-w-0">  <div class="flex items-center gap-2 mb-1"> <span${addAttribute(cn(
      ...
      ), "class")}> ${renderComponent($$result3, "Icon", $$Icon, { "name": "lucide:arrow-right", "class": "w-4 h-4" })} </div> </div> </a>`;
    })} </div>`;
  })()} </div> ` })}`} </div> </main> ` })}`;
